...
- 2026-08-14（活动轨迹 P1）：新增被动活动线索闭环。activity_log 扩展 idle_sec/input_active，新增 ignored suggestions；window 工具接入 GetLastInputInfo 获取 idle 秒数。activity 服务按活动/idle/进程/间隔聚合线索，未记录活动按正式台账空白区间虚拟裁剪，记录内 idle 可一键插入两个切点并拆成“原标签 → 摸鱼/暂停标签 → 原标签”。台账页增加半透明虚线活动轨迹轨道，与实体正式片段区分；线索列表支持补记为台账、按轨迹切分、忽略。设置页增加被动活动记录开关、采样间隔、空闲阈值、最短线索。原则：原始轨迹不动，线索可重算，正式台账仅用户确认后写入。
- 2026-08-14（安装升级生命周期 P0-P3）：版本升至 0.2.0。新增 lifecycle 深模块：首次安装/升级/普通启动识别、升级前 SQLite backup、schema/app 版本降级阻止、migration 失败恢复安排、app_metadata；自定义证据库失联时禁止假空库和继续写入，支持重新定位已有证据库、relative_path 重映射、从 meta 重建索引；配置安全导入导出；新增 GitHub Release/JSON manifest 更新检查、HTTPS 下载、SHA256 强校验、用户确认安装；新增“系统”页集中展示诊断、证据连接、配置迁移、备份恢复、更新状态；发行脚本增加 preflight 与安装包 sha256，NSIS 手动卸载询问本地数据保留，外置证据永不删除。实测 0.1.0→0.2.0 自动备份成功、schema 7、40 测试通过、ASAR 4162 项 forbidden=[]、0.2.0 win-unpacked packaged 诊断通过。完整规范见 plan/RELEASE_LIFECYCLE.md。
...
- 2026-08-11（记录器 P1 消息状态系统）：记录器升级为桌面状态提示层。新增 IPC：recorder:setMessageMode、recorder:showMessage；新增事件 recorder:message。收起态基础宽度 92×54，消息态临时扩到 220×54，2 秒左右自动恢复，不打断 ledger 计时。renderer 新增 messageBadge 与 showMessage 队列逻辑，展开态在底部消息胶囊显示，收起态隐藏状态线并显示消息胶囊。已接入：capture:done 显示“截图已保存”；ledger:state-changed 显示“开始记录/已暂停/继续记录/已存档”；todo created 显示“待办 +1”；主进程快捷截图失败显示“截图失败”。API 冒烟：recorder:setCollapsed=92×54，recorder:setMessageMode active=220×54，inactive=92×54。后续修正：仅靠 webContents.send 投递消息不够稳，增加 latestRecorderMessage + recorder:getMessage 轮询兜底；消息源改到主进程集中投递（ledger/evidence/todos emitters），renderer 只负责展示和拉取，避免 recorder 自己依赖多个事件源。
- 2026-08-11（台账剪辑台与明细内联编辑）：后台台账从列表式展示升级为“单轨剪辑台 + 明细抽屉”。剪辑台以 24h 时间尺展示当天 clip，支持鼠标滚轮围绕指针缩放（1x~64x）、Shift+滚轮横向平移、鼠标按住拖动画布；时间刻度随缩放细化，48x 以上到 1 分钟刻度；时间显示补到 HH:mm:ss。clip 颜色只表示标签，碎片段改用虚线边框/统一斜纹 overlay，极短 micro 片段不再强行撑宽，避免视觉侵入相邻片段。节点保存链路修复：有 pause_points 时不再先 ledger:retag 再 applyPausePointPlan，而是一次性提交 baseTagId/detail/points，由后端在同一事务内更新/拆分/合并，避免第一次保存出现“底层短颜色变化、上层旧状态”的双层假影。明细区默认收起，按钮固定在右侧；点击剪辑台 clip 时联动明细，收起态仅展示当前选中片段，展开态定位并内联展开；明细编辑从悬浮窗改为当前片段向下展开，使用当前标签色框住；同一时间只展开一个；“展示全部/收起明细”统一放在明细卡右上角，并加入轻微串联卡片动效。
- 2026-08-11（20 点加班留证提醒）：补 overtimeEvidenceReminder 服务。应用启动时挂载，默认每天 20:00 后检查一次：若当天未提醒且系统 idle 时间小于 reminder.evidenceIdleThresholdSec（默认 300s），弹系统通知“很晚了，还在加班吗？记得截图落盘”，点击打开证据页 panel.html#evidence，同时记录器轻提示“记得留证”。新增设置：reminder.evidenceCheckTime、reminder.evidenceIdleThresholdSec、reminder.lastEvidenceReminderDate。此前只有 reminder 弹窗壳/待办提醒，没有真正实现“20 点后仍有交互则提醒收集加班证据”的规则。
- 2026-08-12（剪辑台默认工作视角）：台账剪辑台仍保留 24h 可拖动画布，但首次打开、切换今天/昨天/日期后，会自动把横向视图定位到 08:00，避免默认从 00:00 展示导致工作时间段靠后。用户手动拖动、Shift+滚轮平移、滚轮缩放后不会被记录刷新强制拉回 08:00。
- 2026-08-12（进行中记录节点拆分）：修复最新进行中记录在明细里修改节点标签后不拆分的问题。ledger.applyPausePointPlan 现在支持对 end_time 为空的当前记录按节点拆分：节点前片段归档为已完成记录，最后一段保持进行中；前端保存失败时会 alert 错误，不再静默。新增单测覆盖“进行中记录可按暂停点拆分，最后一段保持进行中”，测试数扩展到 26 个。
- 2026-08-12（动态 MCP 接入配置）：设置页新增“Agent MCP 接入”卡片，展示 API、动态启动脚本路径和可复制 MCP JSON。新增 src/main/services/mcpConfig.js，应用启动时在 app.getPath('userData')/mcp/ 下生成 mcp.ps1 与 server.mjs；mcp.ps1 设置 UTF-8、NMLM_API、ELECTRON_RUN_AS_NODE、NODE_PATH，并使用当前 process.execPath 启动 MCP server，避免写死安装路径，兼容中文路径与未来 exe 安装到任意目录。新增 IPC server:mcpConfig，electron-builder files 加入 src/mcp/**。
