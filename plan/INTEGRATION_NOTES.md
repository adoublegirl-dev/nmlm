...
- 2026-08-10（记录器 UI 精简）：用户确认核心功能可用后，要求记录器更横向精简。窗口宽度改为 380，默认高度 86；布局压成两行，主行包含 HH:MM:SS、窄标签下拉、开始/暂停合一按钮、停止按钮；按钮为图标式，title 悬浮提示名称。标签下拉改为自定义 dropdown：平时按钮宽 108，展开菜单宽 230，长标签单行省略不换行；菜单展开时窗口临时向上扩高到 260，收起恢复 86。
