...
牛马面板是一个常驻系统托盘的桌面工作记录仪：启动器负责常驻服务，桌面悬浮“记录器”负责开始/暂停/停止计时和标签选择，浏览器面板负责统计、待办、证据和设置管理。
...
```
US-1 记录一段工作
作为打工人，我按 F8 开始记录，被拉去开会回来按 F9，按 1 选"开会"标签归档，全程不到 2 秒。

US-2 加班证据留存
晚上 8 点系统还在活跃，托盘弹出暗号提醒"到点了，该存档了"。我按 F10 截图存证，连截几张，下班前按  打包证据链。
...
| 动作 | 默认键 | 说明 |
|------|--------|------|
| 开始/暂停记录 | `F8` | 记录中再按 = 暂停（保留现场） |
| 结束记录并选标签 | `F9` | 触发标签浮窗 |
| 快捷截图 | `F10` | 立即截全屏入证据库 |
| 打包证据链 | `` | 弹日期选择浮窗，确认后打包 |
| 打开浏览器面板 | 无默认快捷键 | 从托盘菜单打开，避免占用 F12 |
...
冲突处理：注册失败时，托盘弹通知"快捷键 F9 被其他程序占用"，设置页对应输入框标红提示。
...
3. F8 开始，状态点变淡金；F9 结束，弹出标签浮窗，按数字键 1 秒内归档
