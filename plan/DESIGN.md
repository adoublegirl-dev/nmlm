...
牛马面板是一个常驻系统托盘的桌面记录仪：平时默默记下你被撕碎的时间（台账），到点提醒你留下加班的证据（证据链），最终自动生成你的时间账本与证据包。
...
设计含义：**大多数时候用户只接触①②，③按需打开**。Electron 大窗口不再是主界面，降级为设置兜底与特殊情况处理。
