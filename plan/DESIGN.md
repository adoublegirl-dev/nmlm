...
```
US-1 记录一段工作
作为打工人，我按 Ctrl+Alt+1 开始记录，被拉去开会回来按 Ctrl+Alt+2，按 1 选"开会"标签归档，全程不到 2 秒。

US-2 加班证据留存
晚上 8 点系统还在活跃，托盘弹出暗号提醒"到点了，该存档了"。我按 Ctrl+Alt+3 截图存证，连截几张，下班前按 Ctrl+Alt+4 打包证据链。
...
| 动作 | 默认键 | 说明 |
|------|--------|------|
| 开始/暂停记录 | `Ctrl+Alt+1` | 记录中再按 = 暂停（保留现场） |
| 结束记录并选标签 | `Ctrl+Alt+2` | 触发标签浮窗 |
| 快捷截图 | `Ctrl+Alt+3` | 立即截全屏入证据库 |
| 打包证据链 | `Ctrl+Alt+4` | 弹日期选择浮窗，确认后打包 |
| 打开浏览器面板 | `Ctrl+Alt+0` | 拉起系统浏览器访问本地面板 |
...
冲突处理：注册失败时，托盘弹通知"快捷键 Ctrl+Alt+2 被其他程序占用"，设置页对应输入框标红提示。
...
3. Ctrl+Alt+1 开始，状态点变淡金；Ctrl+Alt+2 结束，弹出标签浮窗，按数字键 1 秒内归档
...
- 快捷键注册冲突：先开一个占用 Ctrl+Alt+1 的程序，验证提示
